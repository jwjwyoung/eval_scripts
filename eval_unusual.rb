...
run_params << [1000, conn, [sql_before, sql_after, 2, "remove join"], nil, nil, nil]
...
run_params << [n, conn, sqls.dup, params.dup, index_range_hash.dup, nil]
...
run_params << [n, conn, sqls.dup, params.dup, index_range_hash.dup, ruby_stm]
...
after_sqls = []
before_sqls = []
# generate corresponding queries based on  the parameters
for i in 0...n
  sql = sql_before
  sql = sql.gsub("$3", "(#{params_arr[i][2].join(",")})")
  before_sqls << sql
  sql = sql + " limit #{params_arr[i][2].uniq.length}"
  after_sqls << sql
end
puts "********28**********"
Benchmark.bm do |x|
  x.report { for i in 0...n; execute_sql(conn, before_sqls[i], params_arr[i][0...2], nil, nil); end }
  x.report { for i in 0...n; execute_sql(conn, after_sqls[i], params_arr[i][0...2], nil, nil); end }
end

run_params.sort { |a, b| a[2][3] <=> b[2][3] }.each do |n, conn, sqls, params, index_range_hash, ruby_stm|
  benchmark_queries(n, conn, sqls, params, index_range_hash, ruby_stm)
end
